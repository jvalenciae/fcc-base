...
module Filter
  class StudentsFilterService < ApplicationService
    ...
    def apply_filters(students)
      ...
      students = students.by_lives_with_reinserted_familiar(params[:lives_with_reinserted_familiar])
      students = students.search_by_q(params[:q]).with_pg_search_rank if params[:q].present?
      students
    end
