  ...
  describe 'POST #create' do
    ...
    context 'when the email exists' do
      it 'generates a reset password token for the user' do
        post '/api/v1/auth/passwords', params: { email: user.email }, headers: authenticated_header(user)
        user.reload
    ...
    context 'when the email does not exist' do
      it 'returns error not_found' do
        post '/api/v1/auth/passwords', params: { email: 'invalid_email' }, headers: authenticated_header(user)
  ...
  describe 'PUT #update' do
    ...
    context 'when the token is valid' do
      ...
      it 'returns a success response' do
        put '/api/v1/auth/passwords/update', params: params, headers: authenticated_header(user)
